...
const flowOrgAPI = 'https://flow-info-xe7v2z5oaa-uc.a.run.app'
...
function bindUI() {
    ...
    $('#airdropChb').change(async (e) => {
        let isAirdrop = $(e.currentTarget).prop('checked')
        console.log('Enable Airdrop: ', isAirdrop)
        if (isAirdrop) {
            enableAirdrop()
        } else {
            disableAirdrop()
...
async function isAirdropEnabled() {
    let allowance = await bnkrx.allowance(currentAddress, airdropAddress).call()
...
async function enableAirdrop() {
    amount = await bnkrx.MAX_INT().call()
    bnkrx.approve(airdropAddress, amount).send({ callValue: 0, feeLimit: feeLimit }).then(tx => {
        refresh(tx)
    }).catch(e => {
        txError(e)
    })
...
function disableAidrop() {
    bnkrx.approve(airdropAddress, 0).send({ callValue: 0, feeLimit: feeLimit }).then(tx => {
        refresh(tx)
    }).catch(e => {
        txError(e)
    })
...
async function showStats() {
    try {
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-outline-semi-light default mb-2" onclick="clipCopy('https://bankroll.network/refinery.html?buddy=${currentAddress}');">Click to copy your referral link</a>
        `)
    ...
    try {
        ...
        for (i = 0; i < players.length; i++) {
            ...
            while (true) {
                ...
                try {
                    tx = await airdrop.send(player, perPerson).send({ callValue: 0, feeLimit: feeLimit })
...
async function pluckAirdrop(player, depth, directs, result) {
    if (result == null) {
...
async function fillPlayer(player, retry) {
    try {
        ...
        return { player: player, directs: directs, deposits: tronWeb.fromSun(deposits - payouts) }
    } catch (e) {
        console.error(e)
        await timeout(100)
        if (retry < 5) {
...
const loadNewActivityData = async (target, filter) => {
    const ACTIVITY_EVENT = 'onAirdrop' //'onPurchase'
    ...
    try {
        ...
        while (true) {
            res = await tronWeb.getEventResult(airdropAddress, requestObj)
