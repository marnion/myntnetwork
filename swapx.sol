...
contract SwapX is TRC20, Whitelist {
    ...
    string public constant name = "MYNT Network Extended Liquidity Token";
    string public constant symbol = "MYNTXSWAP"; // solium-disable-line uppercase
    ...
    /**
     * @notice Convert usdt to Tokens.
     * @dev User specifies exact input (msg.value).
     */
    function() external payable {
        usdtToTokenInput(msg.value, 1, msg.sender, msg.sender);
    }

    /**
      * @dev Pricing function for converting between usdt && Tokens.
      * @param input_amount Amount of usdt or Tokens being sold.
      * @param input_reserve Amount of usdt or Tokens (input type) in exchange reserves.
      * @param output_reserve Amount of usdt or Tokens (output type) in exchange reserves.
      * @return Amount of usdt or Tokens bought.
      */
    ...
    /**
      * @dev Pricing function for converting between usdt && Tokens.
      * @param output_amount Amount of usdt or Tokens being bought.
      * @param input_reserve Amount of usdt or Tokens (input type) in exchange reserves.
      * @param output_reserve Amount of usdt or Tokens (output type) in exchange reserves.
      * @return Amount of usdt or Tokens sold.
      */
    ...
    function usdtToTokenInput(uint256 usdt_sold, uint256 min_tokens, address buyer, address recipient) private returns (uint256) {
        require(usdt_sold > 0 && min_tokens > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_bought = getInputPrice(usdt_sold, address(this).balance.sub(usdt_sold), token_reserve);
     ...
     */
    function usdtToTokenSwapInput(uint256 min_tokens) public payable returns (uint256) {
        return usdtToTokenInput(msg.value, min_tokens,msg.sender, msg.sender);
    }

    function usdtToTokenOutput(uint256 tokens_bought, uint256 max_usdt, address buyer, address recipient) private returns (uint256) {
        require(tokens_bought > 0 && max_usdt > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 usdt_sold = getOutputPrice(tokens_bought, address(this).balance.sub(max_usdt), token_reserve);
        // Throws if usdt_sold > max_usdt
        uint256 usdt_refund = max_usdt.sub(usdt_sold);
        if (usdt_refund > 0) {
            buyer.transfer(usdt_refund);
        }
        require(token.transfer(recipient, tokens_bought));
        emit onTokenPurchase(buyer, usdt_sold, tokens_bought);
        trackGlobalStats();
     ...
     * @return Amount of usdt sold.
     */
    function usdtToTokenSwapOutput(uint256 tokens_bought) public payable returns (uint256) {
        return usdtToTokenOutput(tokens_bought, msg.value, msg.sender, msg.sender);
    }


    function tokenTousdtInput(uint256 tokens_sold, uint256 min_usdt, address buyer, address recipient) private returns (uint256) {
        require(tokens_sold > 0 && min_usdt > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 usdt_bought = getInputPrice(tokens_sold, token_reserve, address(this).balance);
        require(usdt_bought >= min_usdt);
        recipient.transfer(usdt_bought);
    ...
    /**
     * @notice Convert Tokens to usdt.
     * @dev User specifies exact input && minimum output.
     * @param tokens_sold Amount of Tokens sold.
     * @param min_usdt Minimum usdt purchased.
     * @return Amount of usdt bought.
     */
    function tokenTousdtSwapInput(uint256 tokens_sold, uint256 min_usdt) public returns (uint256) {
        return tokenTousdtInput(tokens_sold, min_usdt, msg.sender, msg.sender);
    }

    function tokenTousdtOutput(uint256 usdt_bought, uint256 max_tokens, address buyer, address recipient) private returns (uint256) {
        require(usdt_bought > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_sold = getOutputPrice(usdt_bought, token_reserve, address(this).balance);
     ...
     * @param usdt_bought Amount of usdt purchased.
     * @param max_tokens Maximum Tokens sold.
     * @return Amount of Tokens sold.
     */
    function tokenTousdtSwapOutput(uint256 usdt_bought, uint256 max_tokens) public returns (uint256) {
        return tokenTousdtOutput(usdt_bought, max_tokens, msg.sender, msg.sender);
    }


    function trackGlobalStats() private {

        uint256 price = getusdtToTokenOutputPrice(1e6);
     ...
     * @param usdt_sold Amount of usdt sold.
    ...
    function getusdtToTokenInputPrice(uint256 usdt_sold) public view returns (uint256) {
        require(usdt_sold > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        return getInputPrice(usdt_sold, address(this).balance, token_reserve);
    }

    /**
     * @notice Public price function for usdt to Token trades with an exact output.
     * @param tokens_bought Amount of Tokens bought.
     * @return Amount of usdt needed to buy output Tokens.
     */
    function getusdtToTokenOutputPrice(uint256 tokens_bought) public view returns (uint256) {
        require(tokens_bought > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 usdt_sold = getOutputPrice(tokens_bought, address(this).balance, token_reserve);
        return usdt_sold;
    }
    ...
    /**
     * @notice Public price function for Token to usdt trades with an exact output.
     * @param usdt_bought Amount of output usdt.
     * @return Amount of Tokens needed to buy output usdt.
     */
    function getTokenTousdtOutputPrice(uint256 usdt_bought) public view returns (uint256) {
        require(usdt_bought > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        return getOutputPrice(usdt_bought, token_reserve, address(this).balance);
    }
    ...
    function getusdtToLiquidityInputPrice(uint256 usdt_sold) public view returns (uint256){
        ...
        uint256 usdt_reserve = address(this).balance;
        uint256 token_reserve = token.balanceOf(address(this));
    ...
    function getLiquidityToReserveInputPrice(uint amount) public view returns (uint256, uint256){
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 usdt_amount = amount.mul(address(this).balance) / total_liquidity;
    ...
    /**
     * @notice Deposit usdt && Tokens (token) at current ratio to mint SWAP tokens.
    ...
    function addLiquidity(uint256 min_liquidity, uint256 max_tokens)  public payable returns (uint256) {
        ...
        if (total_liquidity > 0) {
            ...
            uint256 usdt_reserve = address(this).balance.sub(msg.value);
            uint256 token_reserve = token.balanceOf(address(this));
            token_amount = (msg.value.mul(token_reserve) / usdt_reserve).add(1);
            uint256 liquidity_minted = msg.value.mul(total_liquidity) / usdt_reserve;
    ...
    /**
     * @dev Burn SWAP tokens to withdraw usdt && Tokens at current ratio.
     * @param amount Amount of SWAP burned.
     * @param min_usdt Minimum usdt withdrawn.
     * @param min_tokens Minimum Tokens withdrawn.
     * @return The amount of usdt && Tokens withdrawn.
     */
    function removeLiquidity(uint256 amount, uint256 min_usdt, uint256 min_tokens) onlyWhitelisted public returns (uint256, uint256) {
        require(amount > 0 && min_usdt > 0 && min_tokens > 0);
        uint256 total_liquidity = _totalSupply;
        require(total_liquidity > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 usdt_amount = amount.mul(address(this).balance) / total_liquidity;
        uint256 token_amount = amount.mul(token_reserve) / total_liquidity;
        require(usdt_amount >= min_usdt && token_amount >= min_tokens);

        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        _totalSupply = total_liquidity.sub(amount);
        msg.sender.transfer(usdt_amount);
