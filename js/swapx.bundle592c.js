...
const bnkrAddress = 'TE96PgpiB7Z154W6BrtxMiU4ABKfjTYnXf'
...
function bindUI() {
    ...
    let calcReserve = async (e) => {
        ...
        console.log('sell-amount-estimate', amount)
        removeEstimate.text(`${numeral(tronWeb.fromSun(amount[0].toNumber())).format('0.000 a').toUpperCase()} usdt + ` + `${numeral(tronWeb.fromSun(amount[1].toNumber())).format('0.000 a').toUpperCase()} BNKR`)
    }
...
async function showUserStats() {
    ...
    if (userSwap > 0) {
        ...
        $('#user-balance-estimate').html(`<h5 class="color-theme-1 mr-2">Staked Value</h5> <h5><span class="text-white">${formatSun(usdt_value)}</span> usdt + ` + `<span class="text-white">${formatSun(amount[1].toNumber())}</span> BNKRX = <span class="text-success">${formatSun(usdt_value * 2)}</span> usdt</h5>`)
        $('#user-balance-estimate-usdt').html(`${approxStr} ${formatSun(usdt_value * 2 * prices.usdt)} USDT`)
    } else {
        $("#user-estimate").html('Add usdt and BNKR liquidity to earn 0.3%')
